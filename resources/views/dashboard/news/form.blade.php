    ...
    <div class="card alert">
        <div class="card-header">
            ...
            <div class="clearfix header-buttons text-right m-t-20">
                <a href="{{route('news.index')}}" class="btn btn-default btn-sm btn-outline"><i class="ti-close"></i> Cancel</a>
                <a href="#" class="btn btn-primary btn-sm {{isset($d)?'update':'save'}}-form" data-callback="{{ route('news.index') }}" data-status="1"  data-target="#main-form"><i class="ti-save"></i> Save</a>
            </div>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="card alert">
                    <div class="card-body">
                        <form id='main-form' data-url="{{ isset($d)?route('news.update', ['news' => $d->id]):route('news.store') }}">
                            @if(isset($d))    
                                @method('PUT')
                            @endif
                                            ...
                                            <div class="card-body">
                                                <div class="basic-form">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" name="name" class="form-control" value="{{ isset($d)?$d->name:''}}" placeholder="news Name" required>
                                                    </div>

                                                    <div class="form-group">
                                                        <label>Type (News or Award)</label>
                                                        <select name="type" class="form-control" required>
                                                            <option value='news'>News</option>
                                                            <option value='award' {{($d->type ?? '')==='award'?'selected':''}}>Service</option>
                                                        </select>
                                                    </div>
